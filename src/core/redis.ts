...
export async function runRedis(action: ServiceAction): Promise<ServiceStatus> {
  ...
  if (action === "start") {
    ...
    if (conflict.inUse) {
      const nextPort = await resolveAvailablePort(config.redis.port, [6380, 6381, 6382]);
      const occupant = conflict.processName ? ` (was used by ${conflict.processName})` : "";
      await setRedisPort(nextPort);
      await appendLog("redis", `port auto-adjusted ${config.redis.port} -> ${nextPort}${occupant}`);
    }
